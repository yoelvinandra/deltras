                    ...
                    <div class="box-body">
					 <div class="col-md-12">
						...
						<form role="form" id="form_input">
                            ...
                            <input type="hidden" id="IDSPONSOR" name="IDSPONSOR">
                            <div class="box-body">
                                <div class="form-group col-md-6">
                                    <h3 style="font-weight:bold;">Informasi Sponsor</h3>
                                    ...
                                    <br>
                                    <label>Logo Sponsor <i style="color:grey;">&nbsp;&nbsp;&nbsp;Wajib</i> </label>
                                    <br>
                                    <div style="display:flex; align-items:center; gap:20px;">
                                        <div>
                                            <img id="previewGambar" src="<?=base_url()?>assets/images/sponsor/no-logo.png" style="border:1px solid #ccc; object-fit: contain;" width="300" height="200">
                                            <input type="file" class="form-control" id="GAMBAR" name="GAMBAR" accept="image/png" style="width:300px;">
                                        </div>
                                        <span>Syarat :<br>- Format wajib PNG<br>- Ukuran maks 300x200 px<br>- Kapasitas gambar maks 500 kb</span>
                                    </div>
                                    <br>
                                    <label>Website <i style="color:grey;">&nbsp;&nbsp;&nbsp;Wajib</i> </label>
...
$(document).ready(function() {
    ...
    $('#dataGrid').DataTable({
		...
		"scrollX"	  : true,
		ajax		  : {
			url    : base_url+'Master/Data/Sponsor/dataGrid',
			dataSrc: "rows",
...
//   ];
//   // Trigger download
//   XLSX.writeFile(wb, 'SPONSOR_'+dateNowFormatExcel()+'.xlsx');
// }
...
$('#GAMBAR').on('change', function(event) {
    ...
    if (file.size > 500 * 1024) {
        Swal.fire({
            title: "Ukuran file maksimal 500 KB",
            type: "warning"
        });
        $(this).val('');
        $('#previewGambar').attr('src', '<?=base_url()?>assets/images/sponsor/no-logo.png');
        return;
    }
...
function hapus(row){
	get_akses_user('<?=$_GET['kode']?>', function(data){
		if (data.HAPUS==1) {
            ...
            if (row) {
    		    Swal.fire({
                		title: 'Anda Yakin Akan Menghapus Sponsor '+row.NAMA+' ?',
                			...
                			if (result.value) {
                              $("#mode").val('hapus');
                    		    $.ajax({
                    		    	type    : 'POST',
                    		    	dataType: 'json',
                    		    	url     : base_url+"Master/Data/Sponsor/hapus",
...
function clearForm(){
	...
	$("#EMAILCP").val("");
	$("#CATATAN").val("");
    $('#previewGambar').attr('src', '<?=base_url()?>assets/images/sponsor/no-logo.png'); // 🔥 tambahan
}
